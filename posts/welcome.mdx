...
If you want to make sound in WereSoCool, you'll need a `.socool` file. At the top of your `.socool` file, you'll need a `starting point`:
<WereSoCool
  readOnly
  hideGutter
...
This expession defines the starting point for our program. Every `.socool` file is essentially a series of operations on a point. 
...
Now, let's make our first cool sound. Since there haven't any other operations in our program, this operation will be applied directly to our `starting point`.

Click play and you'll hear a sine wave at 220hz that lasts for 1 second before fading out. The gain is one and the
sound is panned to the center. 

#### Try changing the number in the Fm operation. Reminder: The number has to be >= 0. You can also change the values in the `starting point and see how it changes the sound`.
...
[Next Tutorial - A Major Scale](/posts/major_scale)
